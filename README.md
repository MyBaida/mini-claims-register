...
- **Input validation is enforced on both frontend and backend.** Policy numbers allow only letters, numbers, hyphens, and periods. Insured names allow only letters, spaces, hyphens, apostrophes, and periods. Loss nature must contain at least one letter and allows only letters, numbers, and basic punctuation.
